		...
		else if (e.type == SDL_KEYDOWN)
		{
			if (!gameOver)
			{
				...
				if (e.key.keysym.sym == SDL_SCANCODE_Q)
					set_next_state(STATE_EXIT);
				if (e.key.keysym.sym == SDL_SCANCODE_O){
					set_next_state(STATE_ONE_PLAYER_MODE);
					changeToOnePlayerMode = true;
				}
			...
			else if (gameOver)
			{
				if (e.key.keysym.sym == SDL_SCANCODE_Q)
					set_next_state(STATE_EXIT);
				if (e.key.keysym.sym == SDL_SCANCODE_C)
				{
					gameOver = false;
					board.replay();
				}
	...
	for (int r = 1; r <= MAXROWS; r++)
	{
		for (int c = 1; c <= MAXCOLS; c++)
		{
			if (board.m_grid[r - 1][c - 1] != NULL){
				clip = board.m_grid[r - 1][c - 1]->spriteSheetQuadrant();
				spriteSheet.render(renderer, 14 + (c - 1)*GRIDSPACE_WIDTH, 5 + (r - 1)*GRIDSPACE_HEIGHT, 72, 72, &clip);
			}
